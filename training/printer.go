...
func (p *StatsPrinter) PrintProgress(n *deep.Neural, validation Examples, elapsed time.Duration, iteration, iterations int) (loss float64) {
	loss = crossValidate(n, validation)
	fmt.Fprintf(p.w, "%d(%d)\t%s\t%.4f\t%s\n",
		iteration,
		iterations,
		elapsed.String(),
		loss,
